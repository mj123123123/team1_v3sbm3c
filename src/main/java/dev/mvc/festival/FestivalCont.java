...
//	    request.setAttribute("fcateVO", fcateVO);
	  ...
	  public ModelAndView create(HttpServletRequest request, HttpSession session, FestivalVO festivalVO) {
	    ...
	    if (adminProc.isAdmin(session)) { // 관리자로 로그인한경우
	      ...
	      if (Tool.checkUploadFile(file1) == true) { // 업로드 가능한 파일인지 검사
	        ...
	        // mav.addObject("contentsno", festivalVO.getContentsno()); // redirect parameter 적용
	        // ------------------------------------------------------------------------------
	  ...
	  /**
	   * 전체 목록, 관리자만 사용 가능
	   * http://localhost:9093/festival/list_all.do
	   * @return
	   */
...
//	    return mav;
	//  }
	  ...
	  public ModelAndView list_by_fcateno(FestivalVO festivalVO) {
	     ...
	     */
	    String paging = festivalProc.pagingBox(festivalVO.getFcateno(), festivalVO.getNow_page(), festivalVO.getWord(), "list_by_fcateno.do", search_count);
	  ...
	  * @return 페이징용으로 생성된 HTML/CSS tag 문자열
	  */
	   String paging = festivalProc.pagingBox(festivalVO.getFcateno(), festivalVO.getNow_page(), festivalVO.getWord(), "list_by_fcateno_grid.do", search_count);
	   mav.addObject("paging", paging);
	  ...
	  /**
	   * 조회
	   * http://localhost:9093/festival/read.do?contentsno=17
	   * @return
	   */
	  ...
	  /**
	   * MAP 등록/수정/삭제 처리
	   * http://localhost:9093/festival/map.do
	   * @param festivalVO
	   * @return
	   */
	  ...
	  public ModelAndView update_text(HttpSession session, FestivalVO festivalVO) {
	    ...
	    // URL에 파라미터의 전송
	    // mav.setViewName("redirect:/festival/read.do?contentsno=" + festivalVO.getContentsno() + "&fcateno=" + festivalVO.getFcateno());             
	  ...
	  public ModelAndView update_file(HttpSession session, FestivalVO festivalVO) {
	    ...
	    if (this.adminProc.isAdmin(session)) {
	      ...
	      FestivalVO contentsVO_old = festivalProc.read(festivalVO.getContentsno());
	  ...
	  /**
	   * Gallery 전체 이미지 출력
	   * http://localhost:9093/festival/list_all_gallery.do
	   * @return
	   */
